...
def event_name_for(relative_folder: Path, project_name: str, mode: str) -> str:
    parts = relative_folder.parts
    if mode == "parent":
        return parts[-2] if len(parts) >= 2 else project_name
    if mode == "disc":
        return parts[0] if parts else project_name
    return project_name


def main() -> int:
    script_dir = Path(__file__).resolve().parent
    parser = argparse.ArgumentParser(description="iMovieHD2FCP v1.0 alpha batch converter.")
    parser.add_argument("archive_root", type=Path)
    parser.add_argument("output_root", type=Path)
    parser.add_argument("--parser", dest="parser_script", type=Path, default=script_dir / "imoviehd_parser.py")
    parser.add_argument("--converter", dest="converter_script", type=Path, default=script_dir / "imoviehd_v1_converter.py")
    parser.add_argument(
        "--event-mode",
        choices=("project", "parent", "disc"),
        default="project",
        help=("Final Cut Event naming: project=one Event per project; "
              "parent=use the project folder's parent; disc=use the first archive folder."),
    )
    ...
    for index, project_file in enumerate(projects, start=1):
        ...
        with log_file.open("a", encoding="utf-8") as log_handle:
            ...
            if error is None and timeline_json is not None:
                ...
                if converter_code != 0:
                    error = f"v1 converter failed with exit code {converter_code}"
                else:
                    fcpxml = locate_fcpxml(project_output, started_ns)
                    if fcpxml is None:
                        error = "v1 converter completed but no FCPXML was found"
