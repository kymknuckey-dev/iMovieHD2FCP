#!/usr/bin/env python3
"""iMovie HD -> modern media + import-safe FCPXML (v1.0 RC2).
...
def embedded_creation_time(path: Path) -> tuple[float | None, str | None]:
    """Read an embedded container creation date when ffprobe exposes one."""
    try:
    ...
    for key in ("creation_time", "date"):
        ...
        if parsed is not None:
            return parsed, f"embedded {key}"
    return None, None


def timestamp_provenance(path: Path) -> dict[str, Any]:
    """Return every available source date and the selected archival date.

    Selection order:
      1. Embedded container creation time.
      2. macOS filesystem creation time.
      3. Filesystem modification time.

    The manifest records both the raw candidates and the decision.
    """
    stat = path.stat()
    birth = getattr(stat, "st_birthtime", None)
    birth_value = float(birth) if birth and birth > 0 else None
    modified_value = float(stat.st_mtime) if stat.st_mtime > 0 else None
    embedded_value, embedded_reason = embedded_creation_time(path)

    if embedded_value is not None:
        selected = embedded_value
        reason = embedded_reason or "embedded creation time"
    elif birth_value is not None:
        selected = birth_value
        reason = "filesystem creation time"
    elif modified_value is not None:
        selected = modified_value
        reason = "filesystem modification time"
    else:
        selected = datetime.now(tz=timezone.utc).timestamp()
        reason = "conversion time fallback"

    return {
        "filesystem": {
            "created": _iso_utc(birth_value),
            "modified": _iso_utc(modified_value),
        },
        "embedded": {
            "creation_time": _iso_utc(embedded_value),
            "source": embedded_reason,
        },
        "selected": {
            "value": _iso_utc(selected),
            "unix_seconds": selected,
            "reason": reason,
        },
    }


def selected_timestamp(provenance: dict[str, Any]) -> float:
    try:
        return float(provenance["selected"]["unix_seconds"])
    except (KeyError, TypeError, ValueError):
        return datetime.now(tz=timezone.utc).timestamp()


def source_creation_time_from_provenance(provenance: dict[str, Any]) -> str:
    value = provenance.get("selected", {}).get("value")
    return str(value or datetime.now(tz=timezone.utc).isoformat().replace("+00:00", "Z"))


def preserve_file_dates(
    output: Path,
    provenance: dict[str, Any],
) -> None:
    """Apply the selected archival timestamp to the converted file."""
    timestamp = selected_timestamp(provenance)
    ...
    setfile = shutil.which("SetFile") or (
        "/usr/bin/SetFile" if Path("/usr/bin/SetFile").exists() else None
    )
    if setfile:
        local = datetime.fromtimestamp(timestamp).strftime("%m/%d/%Y %H:%M:%S")
        subprocess.run(
            [setfile, "-d", local, str(output)],
            check=False,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        subprocess.run(
            [setfile, "-m", local, str(output)],
            check=False,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
...
def clean_duplicate_suffix(name: str) -> str:
    """Convert iMovie duplicate suffixes such as '/1' into a common base name."""
    return re.sub(r"/\d+$", "", name).strip() or name


def assign_clean_asset_names(plans: list[Any]) -> None:
    """Give duplicate Browser assets readable, stable names.

    The first occurrence keeps its base name; later occurrences become
    ``Name (2)``, ``Name (3)``, and so on. Source filenames and UIDs are not
    changed.
    """
    totals: dict[str, int] = {}
    for plan in plans:
        base = clean_duplicate_suffix(plan.original_asset_name)
    ...
    for plan in plans:
        ...
        plan.asset_name = base if occurrence == 1 else f"{base} ({occurrence})"
...
def collect_media(data: dict[str, Any], converted_dir: Path, *, allow_missing: bool) -> tuple[list[MediaPlan], dict[str, MediaPlan]]:
    ...
    for source_text, info in grouped.items():
        ...
        plan = MediaPlan(
            ...
            probe=asdict(probe),
            timestamp=timestamp_provenance(source) if exists else {
                "filesystem": {"created": None, "modified": None},
                "embedded": {"creation_time": None, "source": None},
                "selected": {"value": None, "unix_seconds": None, "reason": "source missing"},
            },
    ...
    assign_clean_asset_names(plans)
    return plans, by_source


def ffmpeg_command(source: Path, output: Path, probe: Probe, timestamp: dict[str, Any]) -> list[str]:
    ...
    cmd += [
        "-metadata", f"creation_time={source_creation_time_from_provenance(timestamp)}",
...
def convert_plans(plans: list[MediaPlan], *, force: bool) -> None:
    for index, plan in enumerate(plans, start=1):
        ...
        try:
            run(ffmpeg_command(source, temp, probe, plan.timestamp))
            os.replace(temp, output)
            preserve_file_dates(output, plan.timestamp)
...
def collect_audio(data: dict[str, Any], converted_dir: Path, *, allow_missing: bool) -> list[AudioPlan]:
    ...
    for index, item in enumerate(data.get("music") or [], start=1):
        ...
        original_name = str(item.get("name") or source.stem or f"Soundtrack {index}")
...
def main() -> None:
    ...
    manifest_path.write_text(json.dumps({
        ...
        "timestamp_policy": "embedded creation time, then filesystem creation time, then filesystem modification time; every candidate and selection reason recorded",
