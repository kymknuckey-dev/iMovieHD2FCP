#!/usr/bin/env python3
"""Build combined, multi-Event FCPXML imports from completed v1 conversions.
...
def copy_resources(
    ...
    for clone in copied:
        remap_resource_references(clone, old_to_new)
        destination_resources.append(clone)

    return old_to_new, next_resource_number, len(copied)
...
def build_combined(
    ...
    for row in rows:
        ...
        mapping, next_resource_number, resource_count = copy_resources(
            source_root,
            resources,
            next_resource_number,
        )
    ...
    lines = [
        "iMovieHD2FCP v1 Archive Import Build",
        "====================================",
        "",
