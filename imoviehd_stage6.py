#!/usr/bin/env python3
"""iMovie HD -> modern media + import-safe FCPXML (Stage 6).

Stage 6 builds on the stable Stage 3 import. It mirrors Apple-generated FCPXML by
...
left to Final Cut to infer naturally. Separate soundtrack audio is converted and restored as a connected audio clip. Rendered transitions, effects, and titles remain preserved as ordinary media clips.

Requirements:
    ffmpeg and ffprobe on PATH

Example:
    python3 imoviehd_stage6.py \
...
def build_fcpxml(data: dict[str, Any], by_source: dict[str, MediaPlan], audio_plans: list[AudioPlan], project_name: str) -> ET.Element:
    ...
    ET.SubElement(sequence, "note").text = (
        "Stage 6: Event Browser media, embedded clip audio, and separate iMovie HD soundtrack restored. Rendered transitions, effects, and title clips are preserved as media."
    )
...
def main() -> None:
    ...
    xml_path = output_dir / f"{safe_name(project_name)}-Stage6.fcpxml"
    ...
    root = build_fcpxml(data, by_source, audio_plans, project_name)
