#!/usr/bin/env python3
"""iMovie HD -> modern media + import-safe FCPXML (Stage 4).

Stage 4 builds on the stable Stage 3 import. It creates default Event browser clips
for every converted asset and explicitly enables source audio components on timeline clips.
Editable transitions, titles, and background music remain disabled for this stage.

Requirements:
    ffmpeg and ffprobe on PATH

Example:
    python3 imoviehd_stage4.py \
      "output/Small World-timeline.json" \
      --output-dir "output/Small World Stage 4"
"""
...
def ffmpeg_command(source: Path, output: Path, probe: Probe) -> list[str]:
    ...
    # Stage 4 targets PAL SD. Preserve interlacing and lower-field-first DV behaviour.
    if probe.has_video:
        cmd += [
            "-map", "0:v:0",
...
def build_fcpxml(data: dict[str, Any], by_source: dict[str, MediaPlan], project_name: str) -> ET.Element:
    ...
    ET.SubElement(sequence, "note").text = (
        "Stage 4: Event browser clips and embedded source audio enabled; background music and editable transitions disabled."
    )
    ...
    for item in timeline:
        ...
        if original_probe.has_audio:
            channels = max(1, int(original_probe.audio_channels or 2))
            src_channels = ", ".join(str(n) for n in range(1, channels + 1))
...
def validate_result(xml_path: Path, plans: list[MediaPlan]) -> list[str]:
    ...
    for clip in clips:
        ref = clip.get("ref")
        asset = root.find(f"./resources/asset[@id='{ref}']")
        if asset is not None and asset.get("hasAudio") == "1":
            if clip.find("audio-channel-source") is None:
                errors.append(f"Audio clip {clip.get('name')} lacks audio-channel-source")
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Stage 4 output folder")
    ...
    xml_path = output_dir / f"{safe_name(project_name)}-Stage4.fcpxml"
